...
router.post("/track", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!doc) {
      ...
      doc.province = (await geo).province;
      doc.district = (await geo).district;
      doc.village = (await geo).village;
      doc.road = (await geo).road;
      doc.lat = (await geo).lat;
      doc.lon = (await geo).lon;
